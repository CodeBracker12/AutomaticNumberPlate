...
def api1():
    ...
    with open('n.png', 'rb') as fp:
        response = requests.post('https://api.platerecognizer.com/v1/plate-reader/', data=dict(regions=regions),
                                 files=dict(upload=fp), headers={'Authorization': 'Token 74f3de8c9e8fd095d2e401169845c567baf17228'})
        response_data = response.json()
        print("Response from external service:", response_data)

        if 'results' in response_data:
            d["numberplate"] = response_data['results'][0]['plate']
            ...
            print("No 'results' key in the response or external service error")
            # You can handle this case by setting default values or taking appropriate action
